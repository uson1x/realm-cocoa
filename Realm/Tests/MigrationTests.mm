...
- (void)assertPropertyRenameError:(NSString *)errorMessage objectSchemas:(NSArray *)objectSchemas
    ...
    NSString *actualError = [RLMRealm migrateRealm:config].localizedDescription;
    XCTAssertTrue([actualError rangeOfString:errorMessage].location != NSNotFound,
                  @"\"%@\" should contain \"%@\"", actualError, errorMessage);
}
